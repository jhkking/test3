...
public class ReviewInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(!ServletFileUpload.isMultipartContent(request)){
			view = request.getRequestDispatcher("views/reviewboard/ReivewError.jsp");
		...
		//업로드된 파일이 있다면
		 if(originalFileName != null) {
			 ...
			 board = new ReviewBoard(title, writer, content, originalFileName, renameFileName);
		 }else {
			 //업로드된 첨부파일이 없다면
			 board = new ReviewBoard(title, writer, content, null, null);
		 }
		 /*int result = new BoardService().insertBoard(board);
		 if(result >0) {*/
		 if(new ReviewBoardService().insertBoard(board)>0) {
			 response.sendRedirect("/semi/rblist?page=1");
		 }else {
			 view = request.getRequestDispatcher("views/board/reviewError.jsp");
