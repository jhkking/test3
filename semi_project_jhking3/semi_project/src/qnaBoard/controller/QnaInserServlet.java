...
/**
 * Servlet implementation class QnaInserServlet
 */
@WebServlet("/qinsert")
public class QnaInserServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public QnaInserServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//업로드된 파일이 있다면
		 if(originalFileName != null) {
			 ...
			 board = new QnaBoard(title, writer, content, originalFileName, renameFileName);
		 }else {
			 //업로드된 첨부파일이 없다면
			 board = new QnaBoard(title, writer, content, null, null);
		 }
		 /*int result = new BoardService().insertBoard(board);
		 if(result >0) {*/
		 if(new QnaService().insertBoard(board)>0) {
