...
<script type="text/javascript">
	 ...
	 function imgclick() {
      ...
      if (img.src.match("heart")) {
         ...
         $.ajax({
        	type:"post",
        	url: "likelist.do",
        	data : {like_member : "${loginUser.member_id}" , like_place_code: "관광지", like_place_no : "${spotOne.s_no}", action1 : "delete"  },
        	dataType: "json",
        	success:function(data){
    	  ...
    	  $.ajax({
        	type:"post",
        	url: "likelist.do",
        	data : {like_member : "${loginUser.member_id}" , like_place_code:"관광지" , like_place_no :"${spotOne.s_no}", action1 : "insert" },
        	dataType: "json",
        	success:function(data){
    ...
    $.ajax({
    	type:"post",
    	url: "likelist.do",
    	data : {like_member : "${loginUser.member_id}" , like_place_code: "관광지", like_place_no : "${spotOne.s_no}", action1 : "none"  },
    	dataType: "json",
    	success:function(data){
    		/*  img.src = "/finalPro/img/love.png"; */
    		 console.log("리스트가져오기!");
	...
	<div id="menu-wrap" class="menu-back cbp-af-header">
		<div class="container">
                  ...
                  <a href="enrollForm.do" class="animsition-link"
					...
					<c:if test="${!empty sessionScope.loginUser }">
						...
						<a href="#" class="animsition-link"
							style="cursor: pointer;"> ${loginUser.member_name } 님</a>&nbsp;
			...
			<ul class="slimmenu" >
				...
				<li><a href="#">Review</a>
					<ul class="submenu-back">
						<li><a href="rlist.do" class="animsition-link" onclick="loginCheck();">Review</a>
						</li>
						<li><a href="flist.do" class="animsition-link" onclick="loginCheck();">Feedback</a>
						</li>
					</ul></li>
					<li><a href="" class="animsition-link">Rootconsulting</a>
				<ul class="submenu-back">
						<li><a href="datecheck.do" class="animsition-link"  onclick="loginCheck();">new root　</a>
						</li>
						<li><a href="myrootlist.do?id=${loginUser.member_id}" class="animsition-link"   onclick="loginCheck();">my root　　</a>
						</li>
					</ul></li>
			...
			<div class="center" style="margin-left: 25%; word-break: break-all">
				<div class="image-auto-wraper right">
						...
						<div style="position: relative; z-index: 1;">
								<img src="images/${spotOne.s_original_filename}" alt=""  style="width:500px;height:350px">
						</div>
					</div>
					<div style="position : absolute;" >
					...
					<div style="position:absolute;top:60%">
						<font size="3px">${spotOne.s_content }</font>
					</div>
				</div>

				<div class="left" style="overflow-y: auto;">
					<div id="spotreplyview">
						<c:forEach var="sr" items="${replyList}" varStatus="status">
							<font size="2px;"> ▶${sr.sp_content} [${sr.sp_writer}]
								-${sr.sp_date}
								<c:if test="${loginUser.member_id eq sr.sp_writer or loginUser.member_id eq 'admin'}">
										
										<a style=color:black;  href="deletespotreply.do?sp_no=${sr.sp_no}&sp_ref_no=${sr.sp_ref_no}">&nbsp;&nbsp;X</a>
					...
					<c:if test="${!empty loginUser.member_id }">
							...
							<!--  <button class="bt  animated-button victoria-two">Good !</button>  -->
							
						<form name="rform" id="rform" action="insertspotreply.do">
							&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="sp_content"
								placeholder="댓글입력..."
								style="border-top: none; border-left: none; border-right: none;"
								name="sp_content">
				
							<input type="hidden" id="sp_ref_no" name="sp_ref_no"
								value="${spotOne.s_no}"> <input type="hidden"
								id="sp_writer" name="sp_writer" value="${loginUser.member_id}">
							<input type="submit" id="replybtn" value="등록">
						</form>
						<c:if test="${loginUser.member_id eq spotOne.s_writer or loginUser.member_id eq 'admin' }">
										
											<input type="submit" id="replybtn" value="삭제"
											onclick="document.location.href='deletespot.do?no=${spotOne.s_no}&nation1=${spotOne.s_nation1}'">
