...
<script type="text/javascript">
	 ...
	 function imgclick() {
		 loginCheck();
	      ...
	      if (img.src.match("heart")) {
	    	  console.log("${loginUser.member_id}"+  ",  싫어요" );
	         $.ajax({
	        	type:"post",
	        	url: "likelist.do",
	        	data : {like_member : "${loginUser.member_id}" , like_place_code: "숙박", like_place_no : "${hotelOne.h_no}", action1 : "delete"  },
	        	dataType: "json",
	        	success:function(data){
	    	  ...
	    	  console.log("${loginUser.member_id}"+",  좋아요");
	    	  $.ajax({
	        	type:"post",
	        	url: "likelist.do",
	        	data : {like_member : "${loginUser.member_id}" , like_place_code:"숙박" , like_place_no :"${hotelOne.h_no}", action1 : "insert" },
	        	dataType: "json",
	        	success:function(data){
 ...
 $.ajax({
 	type:"post",
 	url: "likelist.do",
 	data : {like_member : "${loginUser.member_id}" , like_place_code: "숙박", like_place_no : "${hotelOne.h_no}", action1 : "none"  },
 	dataType: "json",
 	success:function(data){
 		/*  img.src = "/finalPro/img/love.png"; */
 		 console.log("리스트가져오기!");
...
<body>
				...
				<div class="image-auto-wraper right">
					<div style="position : absolute" >
						<div style="position: relative; z-index: 1;">
								<img src="images/${hotelOne.h_original_filename}" alt="" style="width:500px;height:350px">
						</div>
					</div>
					<div style="position : absolute;" >
					...
					<div style="position:absolute;top:60%">
						<font size="3px">${hotelOne.h_content }</font>
					</div>
				</div>

				<div class="left" style="overflow-y: auto;">
					<div id="hotelreplyview">
						<c:forEach var="hl" items="${replyList}" varStatus="status">
							<font size="2px;"> ▶${hl.ho_content} [${hl.ho_writer}]
								-${hl.ho_date} 
								<c:if test="${loginUser.member_id eq hl.ho_writer }">
										
										<a style=color:black;  href="deletehotelreply.do?ho_no=${hl.ho_no}&ho_ref_no=${hl.ho_ref_no}">&nbsp;&nbsp;X</a>
	...
	<!-- 						<img src="img/star.png"> -->
						...
						<form name="rform" id="rform" action="inserthotelreply.do">
							&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="ho_content"
								placeholder="댓글입력..."
								style="border-top: none; border-left: none; border-right: none;"
								name="ho_content">
				
							<input type="hidden" id="ho_ref_no" name="ho_ref_no"
								value="${hotelOne.h_no}"> <input type="hidden"
								id="ho_writer" name="ho_writer" value="${loginUser.member_id}">
							<input type="submit" id="replybtn" value="등록">
						</form>
						<c:if test="${loginUser.member_id eq hotelOne.h_writer or loginUser.member_id eq 'admin' }">
										
											<input type="submit" id="replybtn" value="삭제"
											onclick="document.location.href='deletehotel.do?no=${hotelOne.h_no}&nation1=${hotelOne.h_nation1}'">
